...
async function ensureDataFiles() {
    ...
    await ensureFile(OTP_FILE, []);
    await ensureFile(SAVINGS_FILE, []);
    await ensureFile(AUDIT_FILE, []);
    await ensureFile(WEEKLY_UPDATES_FILE, []);
...
async function appendAudit(actorType, actorId, action, detail) {
...
function summarizeAnalytics(savings, users, sessions) {
    ...
    const topCategory = [...categoryTotals.entries()].sort((a, b) => b[1] - a[1])[0]?.[0] || 'No tracked category yet';
...
const server = createServer(async (req, res) => {
    ...
    try {
        ...
        if (req.method === 'GET' && url.pathname === '/api/store') {
            const store = await readJson(OFFERS_FILE);
        ...
        if (req.method === 'POST' && url.pathname === '/api/auth/request-otp') {
            ...
            const otpRequests = await readJson(OTP_FILE);
        ...
        if (req.method === 'GET' && url.pathname === '/api/account/summary') {
            ...
            const savings = await readJson(SAVINGS_FILE);
            ...
            const topCategory = [...new Map(history.map((entry) => [entry.categoryName, history.filter((item) => item.categoryName === entry.categoryName).length])).entries()]
        ...
        if (req.method === 'GET' && url.pathname === '/api/admin/dashboard') {
            const [users, sessions, savings, audit, weeklyUpdates, schedule] = await Promise.all([
                ...
                readJson(SAVINGS_FILE),
                readJson(AUDIT_FILE),
                readJson(WEEKLY_UPDATES_FILE),
                readJson(SCHEDULE_FILE),
            ]);
            const dashboard = {
